...
Nav, "flip the card" hint, collapsible contact form and footer, styled like the website. The text comes from `../assets/data/{lang}/profile.json` and `ui.json` (key `ardemo`), with FR defaults in the markup for `npm run serve`. The overlay root has `pointer-events:none`, so taps reach the AR canvas.
